...
import numpy as np
import catboost
from utils import preprocess_df, make_gold_fts, make_fts
...
def load_model(model_name='Catboost (Наивысшая скорость)'):
    if model_name == 'Catboost (Наивысшая точность)':
        cat_fts = ['Поставщик',
                         'Материал',
        ...
        return model.load_model('model_best.cbm'), cat_fts
    
    elif model_name == 'Catboost (Наивысшая скорость)':
        ...
        return model.load_model('fast_catboost.cb'), cat_fts


@st.cache_data(show_spinner=False)
def load_data(model_type='Catboost (Наивысшая скорость)'):
    df_train = pd.read_csv('data/train_AIC.csv', dtype={
                                                        'Поставщик': np.int16,
                                                        'Материал': np.uint16,
    ...
    df_test = pd.read_csv('data/test_AIC.csv')

    if model_type == 'Catboost (Наивысшая точность)': 
        df_train, df_test = preprocess_df(df_train), preprocess_df(df_test)
        df_train, df_test = make_gold_fts(df_train), make_gold_fts(df_test)

        df_train = make_fts(df_train, df_test).fillna(-1).drop_duplicates()
        df_test = make_fts(df_test, df_train).fillna(-1)

    elif model_type == 'Catboost (Наивысшая скорость)':
        df_train['diff'] = df_train['До поставки'] - df_train['Длительность']
        df_train = df_train.fillna(-1).drop_duplicates()
        df_test['diff'] = df_test['До поставки'] - df_test['Длительность']

    return df_train, df_test

@st.cache_data(show_spinner=False)
def process_data(test, model_type='Catboost (Наивысшая скорость)'):
    if model_type == 'Catboost (Наивысшая точность)': 
        test = preprocess_df(test)
        test = make_gold_fts(test)

        test = make_fts(test, df_train).fillna(-1)

    elif model_type == 'Catboost (Наивысшая скорость)':
        test['diff'] = test['До поставки'] - test['Длительность']
        test = test.fillna(-1)
...
def get_preds(df):
    ...
    df[cat_fts] = df[cat_fts].astype(str)
    
    return model.predict_proba(Pool(df[model.feature_names_], cat_features=cat_fts))
...
with st.spinner('Загружаем модель...'):
    model, cat_fts = load_model(model_name=option_model) # потом изменить на option

with st.spinner('Загружаем данные...'):
    df_train, df_test = load_data(model_type=option_model) # потом изменить на option
...
if st.session_state.clicked1 or st.session_state.clicked2 or st.session_state.clicked:
    ...
    test = process_data(test, model_type=option_model)

    with tab1:  

        preds = get_preds(test)

        if len(preds) != 1:
            predicted_class = ['Просрочка' if x else 'В срок' for x in np.argmax(preds, axis=1)]
            test['y'] = [1 if x == 'Просрочка' else 0 for x in predicted_class]
            ...
            test['y'] = 1 if predicted_class == 'Просрочка' else 0

            risk = round(np.min(preds)*100)
            confidence = round(np.max(preds)*100)

        test['Предсказание'] = predicted_class
        ...
        test['Риск'] = test['Риск'].values + \
            np.array([supp_rating_risk[int(supp_stat[str(round(int(x['Поставщик'])))][0])] if str(round(int(x['Поставщик']))) in list(supp_stat.keys()) else 0 for i, x in test.iterrows()]) + \
                np.array([time_to_risk(x) for x in test['До поставки'].values])
  
        test['Уверенность'] = confidence

        st.dataframe(test, height=200)
        option = st.selectbox('Какой семпл для анализа выбрать?', np.arange(test.shape[0]))
        ...
        y_count = test['Предсказание'].value_counts()
        ...
        col3.metric('Средний риск', str(round(test['Риск'].mean(), 2))+'%')
        col4.metric('Средний рейтинг поставщика', str(round(np.mean([int(supp_stat[str(round(int(x['Поставщик'])))][0]) if str(round(int(x['Поставщик']))) in list(supp_stat.keys()) else 0 for i, x in test.iterrows()]), 2))+'⭐')

        st.divider()

        sample = test.iloc[int(option)]
        ...
        shap_values, explainer = get_shap_values(similar_samples)
        ...
        with col1:
            ...
            perm_importance = permutation_importance(model, similar_samples.drop('y', axis=1), similar_samples['y'], n_repeats=5, random_state=42)
        ...
        phk_df['y'] = model.predict(similar_samples)
        ...
        st.button(f'Скачать таблицу с предсказаниями', type='secondary', use_container_width=True)
