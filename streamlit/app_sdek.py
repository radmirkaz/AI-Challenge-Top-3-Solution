...
class CatboostModel:
    ...
    def process_data(self, test, model_type='Catboost (Наивысшая скорость)'):
        if self.model_type == 'Catboost (Наивысшая точность)': 
    ...
    def predict(self, df):
        ...
        df[self.cat_fts] = df[self.cat_fts].astype(str)
        preds = self.model.predict_proba(Pool(df[self.model.feature_names_], cat_features=self.cat_fts))
        return preds
